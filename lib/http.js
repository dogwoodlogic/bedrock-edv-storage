...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {
        config: {controller}
      } = await storage.getConfig({actor: null, id: edvId});
      await bedrock.events.emit('bedrock-edv-storage.document-insert', {
        docId: doc.id,
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {body: doc} = req;
      await storage.update({edvId, doc});
      const {
        config: {controller}
      } = await storage.getConfig({actor: null, id: edvId});
      await bedrock.events.emit('bedrock-edv-storage.document-update', {
        docId: id,
        sequence: doc.sequence,
        invoker,
        controller,
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      await bedrock.events.emit('bedrock-edv-storage.document-get', {
        docId: id,
        sequence: doc.sequence,
        invoker,
        controller,
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const {body: chunk} = req;
      await storage.updateChunk({edvId, docId, chunk});
      const {
        config: {controller}
      } = await storage.getConfig({actor: null, id: edvId});
      await bedrock.events.emit('bedrock-edv-storage.chunk-store', {
        docId,
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      await bedrock.events.emit('bedrock-edv-storage.chunk-get', {
        docId,
        invoker,
        controller,
        chunkIndex,
        edvId: req.params.edvId
      });
      // TODO: need to determine how to filter the recipients to include
      // in the JWE (use specified recipient key in the JWE or invoker ID
      // somehow)
  ...
  app.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      // gets the controller for the event
      const {
        config: {controller}
      } = await storage.getConfig({actor: null, id: edvId});

      await bedrock.events.emit('bedrock-edv-storage.chunk-delete', {
        docId,
        invoker,
        controller,
        chunkIndex,
